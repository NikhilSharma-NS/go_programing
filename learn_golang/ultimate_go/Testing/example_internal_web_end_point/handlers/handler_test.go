...
func TestSendJSON(t *testing.T) {
	...
	t.Log("Given the need to test the SendJSON endpoint.")
	{
		...
		t.Logf("\tTest 0:\tWhen checking %q for status code %d", url, statusCode)
		{
			...
			if u.Name == "Nikhil Sharma" {
				t.Logf("\t%s\tShould have \"Nikhil Sharma\" for Name in the response.", succeed)
			} else {
				t.Errorf("\t%s\tShould have \"Nikhil Sharma\" for Name in the response : %q", failed, u.Name)
			}

			if u.Email == "nkhlsharma050@gmail.com" {
				t.Logf("\t%s\tShould have \"nkhlsharma050@gmail.com\" for Email in the response.", succeed)
			} else {
				t.Errorf("\t%s\tShould have \"nkhlsharma050@gmail.com\" for Email in the response : %q", failed, u.Email)
			}
		}
	}
}
